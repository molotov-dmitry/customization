...
repoadd()
{
    ...
    local repofilename="$(echo "${reponame}-${version}" | tr '[:upper:]' '[:lower:]' | tr ' /.' '-' | sed 's/-$//g')"
...
function repoaddnonfree()
{
    ...
    elif [[ "$(lsb_release -si)" == "Debian" ]] && ( [[ "$(lsb_release -sr)" == 'n/a' ]] || dpkg --compare-versions "$(lsb_release -sr)" ge 12 )
    then
